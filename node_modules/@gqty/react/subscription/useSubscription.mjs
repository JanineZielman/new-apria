import { useForceUpdate, useSelectionsState, useIsomorphicLayoutEffect, isAnySelectionIncluded } from '../common.mjs';
  ...
  const useSubscription = function useSubscription2() {
    ...
    useIsomorphicLayoutEffect(() => {
      ...
      const unsubErrors = scheduler.errors.subscribeErrors((data) => {
        if (isMounted && data.type === "new_error" && !forceUpdate.wasCalled.current && isAnySelectionIncluded(data.selections, hookSelections)) {
          forceUpdate();
        }
      });
      return () => {
