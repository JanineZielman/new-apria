...
import { useBuildSelections, isAnySelectionIncludedInMatrix, useIsomorphicLayoutEffect, isSelectionIncluded, isAnySelectionIncluded } from '../common.mjs';
import { areArraysEqual } from '../utils.mjs';
...
function createUseMetaState(client) {
  ...
  const useMetaState = function useMetaState2(opts = defaultEmptyOpts) {
    ...
    const setStateIfChanged = React.useCallback(
      function setStateIfChanged2(isMounted) {
        if (!isMounted.current)
          return;
        const prevState = stateRef.current;
        const newState = getState(isMounted);
        if (prevState.isFetching !== newState.isFetching || !areArraysEqual(prevState.errors, newState.errors)) {
      ...
      const unsubscribeIsFetching = scheduler.subscribeResolve(
        (promise, selection) => {
          ...
          if (promisesInFly.has(promise))
            return;
          if (hasFilterSelections && !isSelectionIncluded(selection, selectionsToFilter)) {
            return;
          }
